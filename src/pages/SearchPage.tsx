...
function SearchBar({ query, onChange, onClear, onBack }: {
  ...
  return (
    <header className="Search-header sticky top-0 z-40 bg-[var(--color-bg)] border-b border-[var(--color-border)]">
      <div className="flex items-center gap-4 px-4 h-14">
        ...
        {query && (
          <button onClick={onClear} className="text-[var(--color-text-secondary)] text-sm hover:text-[var(--color-text)]">取消</button>
        )}
      </div>
    </header>
  );
...
function SearchResultItem({ item }: { item: any }) {
  ...
  return (
    <button onClick={() => navigate(`/item/${item.id}`)} className="Search-resultItem w-full flex items-center gap-4 p-3 rounded-xl hover:bg-[var(--color-bg-card)] transition-colors active:bg-[var(--color-bg-input)]">
...
function SearchStatus({ isLoading, hasResults, minLen }: {
  ...
  if (minLen >= 1) return <div className="flex flex-col items-center justify-center py-20 text-[var(--color-text-secondary)]"><Search className="w-12 h-12 mb-4 opacity-50" /><p>没有找到相关结果</p></div>;
  return <div className="flex flex-col items-center justify-center py-20 text-[var(--color-text-tertiary)]"><Search className="w-12 h-12 mb-4 opacity-30" /><p>输入关键词搜索有声书</p></div>;
}
